...
let currentExamType = 'all'; // 'all', 'A-Level', or 'GCSE'
...
function parseCSV(csvText) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        if (values[0].includes('National percentage') || values[0] === 'subject') continue;
...
function displayHomePage() {
    ...
    if (currentExamType !== 'all') {
        filteredSubjects = filteredSubjects.filter(key => key.startsWith(currentExamType + ':'));
    }
    
    // Prepare sorted subjects list with optional filtering
    let sortedSubjects = [...filteredSubjects];
    if (currentSort === 'popularity') {
        sortedSubjects.sort((a, b) => subjectStats[b].candidates - subjectStats[a].candidates);
    }
    ...
    const examTypeLabel = currentExamType === 'all' ? 'All Subjects' : currentExamType === 'A-Level' ? 'A-Level Subjects' : 'GCSE Subjects';
        ...
        <div class="home-page">
            <div class="home-header">
                ...
                <div class="exam-type-controls">
                    ...
                    <label class="radio-label"><input type="radio" name="examType" value="all" ${currentExamType === 'all' ? 'checked' : ''}> All</label>
...
function handleSearchInput(e) {
    ...
    if (matches.length > 0) {
        suggestionsDiv.innerHTML = matches.map(subjectKey => {
            const [examType, subject] = subjectKey.split(':');
            return `<div class="suggestion-item" onclick="selectSubject('${subjectKey.replace(/'/g, "\\'")}')">${subject}</div>`
        ).join('');
                ...
                <table class="data-table">
                    <thead>
                        <tr>
                            <th>Year</th>
                            <th>Type</th>
                            ${generateTableHeaders(gradeSystem)}
...
// Generate Pass Rate Chart (pass rate over years)
function generatePassRateChart(data) {
    // keep data in descending year order (most recent first)
...
// Generate Grade Distribution Chart (All grades over years)
function generateGradeDistributionChart(data) {
    // keep data in descending year order (most recent first)
    const examData = data.filter(d => d.data_type !== 'Teacher').slice(0, 15);
    if (examData.length === 0) return '';
    
    const gradeSystem = examData[0].grade_system;
    const grades = getGradeLabels(gradeSystem);
    const maxValue = Math.max(...examData.flatMap(d => {
        const rowGrades = getGradesFromRow(d);
        return grades.map(grade => parseFloat(rowGrades[grade]) || 0);
    }));
    ...
    examData.forEach(row => {
        ...
        grades.forEach(grade => {
            ...
            const height = maxValue > 0 ? (value / maxValue) * 100 : 0;
            const gradeClass = grade.toLowerCase().replace('*', '-star').replace('/', '-');
            
            html += `
                <div class="grade-bar ${gradeClass}" style="height: ${height}%;" title="${grade}: ${value}%"></div>
            `;
        });
...
// Generate Top Grades Chart (top grades over years)
function generateTopGradesChart(data) {
    ...
    const topGradeLabel = gradeSystem === '9-1' ? '9-7' : 'A* + A';
    let html = `
        <div class="chart-wrapper">
...
function formatValue(value) {
    ...
    // Keep '0' as is (it's a valid percentage)
...
// Calculate A*-C rate or 9-4 equivalent
