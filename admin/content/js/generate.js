    ...
    renderIndividualGenerateModal(courseId, courseTitle, tiersWithTopics, voicesData) {
        ...
        for (const tier of tiersWithTopics) {
            ...
            const topicsHTML = tier.topics.map(topic => {
                ...
                const notesCharsStr = this.formatCharCount(topic.notes_char_count);
                const scriptCharsStr = this.formatCharCount(topic.podcast_script_char_count);
                const durationStr = this.formatDuration(topic.podcast_length_seconds);
                    ...
                    <label class="individual-topic-item ${hasContent ? 'has-content' : 'no-content'}">
                        ...
                        <div class="topic-info">
                            ...
                            <div class="topic-stats">
                                <span class="stat ${notesCount > 0 ? 'has' : 'empty'}" title="${notesCharsStr ? `Notes: ${notesCharsStr}` : 'No notes'}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>
                                    ${notesCount} notes${notesCharsStr ? ` · ${notesCharsStr}` : ''}
                                </span>
                                <span class="stat ${hasPodcastScript ? 'has' : 'empty'}" title="${scriptCharsStr ? `Script: ${scriptCharsStr}` : 'No podcast script'}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg>
                                    Script${scriptCharsStr ? ` · ${scriptCharsStr}` : ''}
                                </span>
                                <span class="stat ${hasPodcastFile ? 'has' : 'empty'}" title="${durationStr ? `Audio: ${durationStr}` : hasPodcastFile ? 'Audio file exists (length unknown)' : 'No podcast audio'}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"></path><path d="M19 10v2a7 7 0 0 1-14 0v-2"></path><line x1="12" y1="19" x2="12" y2="23"></line></svg>
                                    Audio${durationStr ? ` · ${durationStr}` : hasPodcastFile ? ' ✓' : ''}
                                </span>
    ...
    renderLocalTtsTopicSelection(course, tiersWithTopics) {
        ...
        for (const tier of tiersWithTopics) {
            ...
            const topicsHTML = tier.topics.map(topic => {
                ...
                const hasPodcastFileLocal = topic.has_podcast_file || false;
                const audioStatHtml = durationStr
                    ? `<span style="font-size:0.75rem; color:#64748b;" title="Audio length">⏱ ${durationStr}</span>`
                    : hasPodcastFileLocal
                        ? `<span style="font-size:0.75rem; color:#64748b;" title="Audio file exists (length unknown)">⏱ ✓</span>`
                        : '';
                const statsHTML = (audioStatHtml || scriptCharsStr || notesCharsStr) ? `
                    <div class="topic-stats" style="display:flex; gap:0.6rem; flex-wrap:wrap; margin-top:0.2rem;">
                        ${audioStatHtml}
                        ${scriptCharsStr ? `<span style="font-size:0.75rem; color:#64748b;" title="Script character count">📝 ${scriptCharsStr}</span>` : ''}
                        ${notesCharsStr ? `<span style="font-size:0.75rem; color:#64748b;" title="Notes character count">📄 ${notesCharsStr}</span>` : ''}
                    </div>` : '';
