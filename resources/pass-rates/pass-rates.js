...
function handlePopState(event) {
    const subjectKey = getSubjectFromURL();
    if (subjectKey) {
        displaySubjectData(subjectKey, false); // false = don't push to history
    } else {
        displayHomePage(false); // false = don't push to history
    }
}
...
function updateURL(subjectKey, pushState = true) {
    ...
    if (pushState) {
        window.history.pushState({ subject: subjectKey }, '', url);
    } else {
        window.history.replaceState({ subject: subjectKey }, '', url);
    }
}
...
async function loadCSVData() {
    try {
        ...
        // Check if URL has a subject parameter
        const subjectKey = getSubjectFromURL();
        if (subjectKey && allSubjects.includes(subjectKey)) {
            // Extract exam type from subject key and set it
            const [examType] = subjectKey.split(':');
            currentExamType = examType;
            displaySubjectData(subjectKey, false);
        } else {
...
function displayHomePage(pushState = true) {
    const resultsContainer = document.getElementById('resultsContainer');
    const navigationBar = document.getElementById('navigationBar');
    
    navigationBar.style.display = 'none';
    
    // Update URL to remove subject parameter
    if (pushState) {
        updateURL(null, true);
    }
...
function selectSubject(subjectKey) {
    ...
    document.getElementById('suggestions').classList.remove('active');
    displaySubjectData(subjectKey, true);
}
...
function generateGradeLegend(grades) {
    const colors = ['#193659', '#234a7a', '#2a5f8a', '#3678AE', '#4a8fc4', '#53C0B8', '#6fd4cc', '#CBD5E1', '#E2E8F0'];
    return grades.map((grade, i) => `
        <span style="display: inline-block; margin: 0 8px;">
            <span style="display: inline-block; width: 12px; height: 12px; background: ${colors[i % colors.length]}; margin-right: 5px; border-radius: 2px;"></span>${grade}
        </span>
    `).join('');
}

// Display subject data - ALL YEARS with CHARTS
function displaySubjectData(subjectKey, pushState = true) {
    ...
    // Update URL with subject parameter
    if (pushState) {
        updateURL(subjectKey, true);
    }
    
    // Update page title
    document.title = `${subjectName} Pass Rates | Lockin.tech`;
        ...
        <div class="subject-display">
            <div class="subject-header">
                <div class="subject-title-row">
                    ...
                    <button class="share-btn" onclick="shareSubject()" title="Copy link to clipboard">
            ...
            <div class="data-note" style="padding: 12px 16px; background: #fff3cd; border-left: 4px solid #ffc107; margin-bottom: 25px; border-radius: 8px;">
            ...
            html += `
                    <thead>
                        <tr class="grade-system-header">
                            <th colspan="100%" style="background: var(--color-primary, #3678AE); color: white; text-align: center; padding: 8px; font-size: 0.9rem;">
...
function generateGradeDistributionChart(data) {
    ...
    examData.forEach(row => {
        ...
        grades.forEach(grade => {
            ...
            html += `
                <div class="grade-bar ${gradeClass}" style="height: ${height}%;" title="${tooltip}">
                    <span class="grade-bar-value">${value > 0 ? value.toFixed(1) + '%' : ''}</span>
                </div>
            `;
        });
    ...
    html += `
                ...
                <div style="text-align: center; margin-top: 15px; font-size: 0.85rem; color: var(--color-gray-500, #6c757d);">
    ...
    if (hasMixedSystems) {
        html += `
                <div style="text-align: center; margin-top: 10px; font-size: 0.8rem; color: #856404; background: #fff3cd; padding: 8px; border-radius: 8px;">
...
// Share subject - copy link to clipboard
